...
При загрузке страницы пользователю предлагается ввести через prompt число. 
    Это число добавляется в массив чисел numbers.
...
После того как пользователь прекратил ввод нажав Cancel, необходимо взять массив введенных чисел, сложить общую сумму всех элементов массива и записать ее в переменную total. 
...
const uncorrectInput = "Было введено не число, попробуйте еще раз.";
const inputMoreNumbers = "Введите хотябы ещё одно число";
...
do {
  ...
  if (isNaN(number)) {
    alert(uncorrectInput);
  } else if (!userInput && numbers.length === 1) {
    alert(inputMoreNumbers);
  } else if (!userInput) {
    break;
  } else {
    numbers.push(number);
